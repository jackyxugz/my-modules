...
class Payment(models.Model):
    ...
    wechat_user_id = fields.Many2one('wxxcx.user', string='微信用户')
    order_id = fields.Many2one('sale.order', string='订单')
    payment_number = fields.Char('支付单号', index=True)
    price = fields.Float('支付金额(元)')
    status = fields.Selection([(key,value) for key,value in defs.PaymentStatus.__dict__.items() if not key.startswith('__')
                               and not callable(key)],
                              string='状态', default=defs.PaymentStatus.unpaid)
    ...
    _sql_constraints = [(
        'wxxcx_payment_payment_number_unique',
    ...
    def create(self, vals):
        vals['payment_number'] = self.env['ir.sequence'].next_by_code('wxxcx.payment_num')
